...
def main():
    ...
    parser.add_argument('--model', type=str, help='Specific model to train (if not provided, train all from config)')
    ...
    if args.model:
        # Train specific model from command line
        model_types = [args.model]
        logger.info(f"Training specific model: {args.model}")
    else:
        # Train all models from config
        model_types = config.model.model_type
        if isinstance(model_types, str):
            model_types = [model_types]
        logger.info(f"Training all models from config: {', '.join(model_types)}")
